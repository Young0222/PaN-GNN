...
class bipartite_dataset(Dataset):  
    ...
    def negs_gen_EP_double_bpr(self,epoch): # pos > neg; neg > non_feedback
        ...
        for j in set(self.train['userId'].values):
            ...
            temp = (torch.tensor(np.random.choice(neg,len(pos)*self.K*epoch,replace=True,p=self.neg_dist[neg]/self.neg_dist[neg].sum()))+self.num_u).long()
            temp2 = (torch.tensor(np.random.choice(non_fb,len(pos)*self.K*epoch,replace=True,p=self.neg_dist[non_fb]/self.neg_dist[non_fb].sum()))+self.num_u).long()
            self.edge_4_tot[self.edge_1==j-1]=temp.view(int(len(temp)/self.K/epoch),self.K,epoch)
...
def gen_top_k_new(data_class, r_hat_p, r_hat_n, K=300):
    ...
    # print("shape: ", r_hat_p.shape, r_hat_n.shape, '\n')  # 6040*3952
    # print("res: ", r_hat_p, r_hat_n, '\n')
    # print("shape reco_n: ", reco_n.shape, '\n')   # may be 6040*300
    # print("reco_n: ", reco_n, '\n')
    # print("reco_p: ", reco_p, '\n')
    # sys.exit()
...
def gen_top_k_new2(data_class, r_hat_p, r_hat_n, K=300):
    ...
    # 按照 正反馈分 进行排序，得到top20
    _, reco = torch.topk(r_hat_p,20)   # may be 6040*20
